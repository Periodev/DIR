...
const Zone5 = preload("res://scripts/LevelConstants_Zone5.gd")

const DEFAULT_LOAD_ZONE: int = 5
...
static func all_levels() -> Array:
	var result: Array = []
	var next_id: int = 1
	for source_levels: Array in [Zone1.LEVELS, Zone2.LEVELS, Zone3.LEVELS, Zone4.LEVELS, Zone5.LEVELS]:
